...
final public class PTabUICmdKtlKstOpenCrtExpTcr extends PTabUICmdKtlKstOpenCrtExpAbs
{
    // ---------------------------
    // final static private String
    
    final static public String STR_TITLETASK = "Export trusted certificate entry as certificate file";
    ...
    public PTabUICmdKtlKstOpenCrtExpTcr(Frame frmOwner, String strTitleAppli)
    {
        super(
            frmOwner, 
            strTitleAppli,
    ...
    **/
    private boolean _queryPreviewResults(String strFormatFileCert)
    {
        ...
        )
        {
            ...
            for (int i=0; i<S_FileExtensionUI.f_s_strsCrtX509Pkcs7.length; i++)
            {
                if (super._strPathAbsFileIO_.toLowerCase().endsWith("." + 
                    S_FileExtensionUI.f_s_strsCrtX509Pkcs7[i].toLowerCase()))
                {
                    blnGotIt = true;
                    break;
                }
        ...
        else if (strFormatFileCert.toLowerCase().compareTo(KTLAbs.f_s_strFormatFileCrtDer.toLowerCase()) == 0)
        {
            ...
            for (int i=0; i<S_FileExtensionUI.f_s_strsCrtX509Der.length; i++)
            {
                if (super._strPathAbsFileIO_.toLowerCase().endsWith("." + 
                    S_FileExtensionUI.f_s_strsCrtX509Der[i].toLowerCase()))
                {
                    blnGotIt = true;
                    break;
                }
        ...
        else if (strFormatFileCert.toLowerCase().compareTo(KTLAbs.f_s_strFormatFileCrtPem.toLowerCase()) == 0)
        {
            ...
            for (int i=0; i<S_FileExtensionUI.f_s_strsCrtX509Pem.length; i++)
            {
                if (super._strPathAbsFileIO_.toLowerCase().endsWith("." + 
                    S_FileExtensionUI.f_s_strsCrtX509Pem[i].toLowerCase()))
                {
                    blnGotIt = true;
                    break;
                }
